...
public class TrainDiagramFactory {
    ...
    public TrainDiagram createDiagram() {
        try {
            TextTemplate name = TextTemplate.createTextTemplate("@{train.attributes['electric']?'E':''}@{train.attributes['diesel']?'M':''}@{type.abbr} @{train.number}", TextTemplate.Language.MVEL);
            TextTemplate completeName= TextTemplate.createTextTemplate("@{train.attributes['electric']?'E':''}@{train.attributes['diesel']?'M':''}@{type.abbr} @{train.number}@if{train.description != ''} @{train.description}@end{}", TextTemplate.Language.MVEL);
