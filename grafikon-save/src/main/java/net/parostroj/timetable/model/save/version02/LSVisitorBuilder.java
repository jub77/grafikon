...
public class LSVisitorBuilder implements LSVisitor {
    ...
    public void visit(LSTrainDiagram lsDiagram) {
        ...
        trainTypeList.getTrainsData(diagram.getRuntimeInfo().getPermissions()).copyValuesTo(diagram.getTrainsData());
    ...
    public void visit(LSNode lsNode) {
        ...
        Node node = diagram.getPartFactory().createNode(lsNode.getUuid());
        node.setType(type);
    ...
    public void visit(LSTimeInterval lsInterval) {
        ...
        NetSegment<?> part = (NetSegment<?>) ids.get(lsInterval.getOwnerId());
