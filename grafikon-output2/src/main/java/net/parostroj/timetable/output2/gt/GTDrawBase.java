...
abstract public class GTDrawBase implements GTDraw {
	...
	private void updateStartAndSize(Graphics2D g) {
		...
		this.stationNameWidth = this.computeInitialStationNameWidth(g, snWidth); // initial
																					// size
																					// ...
		...
		// compute size
		this.size = new Dimension(configSize.width - (this.borderX + this.start.x),
				configSize.height - (this.borderY + this.start.y));
		// time step
	...
	private void paintHourAndHalfHourOnTime(Graphics2D g, int yEnd, double step, int i, int timeToPaint) {
		if (isTimeVisible(timeToPaint)) {
			...
			if (((i & 1) != 1) || step >= minimalSpace) {
				// draw line
				orientationDelegate.drawLine(g, xLocation, orientationDelegate.getStationsStart(start), xLocation,
						yEnd);
			}
		}
	}
	...
	protected void paintTrainOnLineWithInterval(Graphics2D g, boolean paintTrainName, boolean paintMinutes,
			TimeInterval interval, Interval i) {
		...
		GTDrawEventVisitor visitor = new GTDrawEventVisitor() {
			...
			public void visitDiagramEvent(Event event) {
				...
				case ADDED:
					...
					break;
				case ATTRIBUTE:
					if (event.getAttributeChange().checkName(TrainDiagram.ATTR_FROM_TIME, TrainDiagram.ATTR_TO_TIME)) {
